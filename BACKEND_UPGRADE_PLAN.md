...
18. ✅ Build login UI in React:
    ...
    - User Google avatar + display name (visible when logged in)
    - Sign out button (登出 / Logout)
    - All placed in CourseHub nav bar, next to language toggle
...
27. Build a "My Progress" page or panel accessible from Course Hub
...
29. Add visual indicators on Course Hub cards (checkmarks, completion badges)
30. Bilingual support — add i18n keys for all new UI strings (English + Traditional Chinese)
...
| `App.jsx` | ✅ Added auth state management, login/logout UI in CourseHub nav bar, user/onLogin/onLogout props |
...
| File | Changes |
|------|---------|
| `App.jsx` | Pass `user` prop down to Course components (for progress saving) |
| `Course0.jsx` | Add progress saving when eggs are found (if logged in) |
| `DinoEggHatch.jsx` | Add progress saving when game ends |
| `DinoFoodRescue.jsx` | Add progress saving when game ends |
| `DinoHomeSave.jsx` | Add progress saving when game ends |
| `i18n.js` | Add bilingual strings for progress dashboard |
| `src/ProgressDashboard.jsx` | New — user progress display component |
...
*Status: Phases 0-2 complete (Supabase + Google OAuth + login UI working). Next: Phase 3 (progress saving in game components).*
