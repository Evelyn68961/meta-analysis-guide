...
const T = {
  ...
  en: {
    ...
    tabR: "R Code (metafor)",
    tabWebR: "In-Browser Analysis (recommended)",
...
function buildRCode(studies, bin, effectType, model, modColumns) {
  ...
  const modLines = (modColumns || []).map(col => {
    const rName = col.replace(/[^a-zA-Z0-9_]/g, "_").toLowerCase();
    const vals = studies.map(s => {
      const v = (s.moderators || {})[col] || "";
      // If all values are numeric, don't quote; otherwise quote as character
      return v;
    });
    const allNumeric = vals.every(v => v !== "" && !isNaN(Number(v)));
    if (allNumeric) {
      return `  ${rName} = c(${vals.map(v => v || "NA").join(", ")})`;
    }
...
function Step3({ project, analysis, lang }) {
  ...
  const rCode = buildRCode(inc, bin, analysis.effectType, analysis.model, modColumns);

  const tabs = [
    { key: "webr", icon: "🧪", label: tx.tabWebR },
    { key: "online", icon: "🌐", label: tx.tabOnline },
    { key: "r", icon: "📟", label: tx.tabR },
  ];
      ...
      <div style={{ display: "flex", gap: 0, marginBottom: 20, borderRadius: 10, overflow: "hidden", border: `1px solid ${LIGHT_BORDER}` }}>
        {tabs.map((t, i) => (
          <button key={t.key} onClick={() => setTab(t.key)} style={{
            flex: 1, padding: "10px 12px", fontSize: 12, fontWeight: 600, fontFamily: FONT,
            background: tab === t.key ? `${CRIMSON}10` : CARD_BG,
            color: tab === t.key ? CRIMSON : MUTED,
            border: "none",
            borderRight: i < tabs.length - 1 ? `1px solid ${LIGHT_BORDER}` : "none",
            cursor: "pointer",
            transition: "all 0.2s",
          }}>
            {t.icon} {t.label}
