...
function SectionLabel({ text }) {
  return (
    <div style={{ display: "inline-flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
      ...
      <span style={{ fontSize: 11, fontWeight: 600, letterSpacing: 2, textTransform: "uppercase", color: TEAL, fontFamily: "'Outfit', 'Noto Sans TC', sans-serif" }}>{text}</span>
    </div>
  );
}

function SectionTitle({ children }) {
  return <h2 style={{ fontFamily: "'Source Serif 4', 'Noto Sans TC', serif", fontSize: "clamp(28px, 5vw, 40px)", fontWeight: 700, color: DARK, lineHeight: 1.2, marginBottom: 16 }}>{children}</h2>;
}

function Paragraph({ children, style = {} }) {
  return <p style={{ fontSize: 16, lineHeight: 1.75, color: MUTED, maxWidth: 640, fontFamily: "'Outfit', 'Noto Sans TC', sans-serif", ...style }}>{children}</p>;
}

const btnPrimary = { background: TEAL, border: "none", color: "#FFF", padding: "12px 28px", borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "'Outfit', 'Noto Sans TC', sans-serif", transition: "all 0.2s" };
const btnSecondary = { background: "transparent", border: `1.5px solid ${TEAL}`, color: TEAL, padding: "11px 26px", borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "'Outfit', 'Noto Sans TC', sans-serif" };
  ...
  return (
    <div style={{ background: CARD_BG, borderRadius: 20, border: `1px solid ${LIGHT_BORDER}`, padding: "32px 28px", boxShadow: "0 2px 20px rgba(0,0,0,0.04)" }}>
      <div style={{ textAlign: "center", marginBottom: 24 }}>
        <h3 style={{ fontFamily: "'Source Serif 4', 'Noto Sans TC', serif", fontSize: 20, fontWeight: 600, color: DARK, marginBottom: 6 }}>
          {phase === "scattered" && t("combinerScattered")}
          {phase === "combining" && t("combinerCombining")}
          {phase === "combined" && t("combinerCombined")}
        </h3>
        <p style={{ fontSize: 14, color: MUTED, fontFamily: "'Outfit', 'Noto Sans TC', sans-serif" }}>
      ...
      <div style={{ position: "relative", height: 260, background: "#FAFAF7", borderRadius: 14, border: `1px solid ${LIGHT_BORDER}`, overflow: "hidden", marginBottom: 20 }}>
        <div style={{ position: "absolute", bottom: 8, left: 0, right: 0, display: "flex", justifyContent: "space-between", padding: "0 16px", fontSize: 10, color: "#B0AFAA", fontFamily: "'Outfit', 'Noto Sans TC', sans-serif" }}>
        ...
        <div style={{ position: "absolute", left: "50%", top: 6, transform: "translateX(-50%)", fontSize: 9, color: "#BBB", fontFamily: "'Outfit', 'Noto Sans TC', sans-serif" }}>{t("combinerNoEffect")}</div>
        {studies.map((st, i) => {
        ...
        {hoveredStudy !== null && phase === "scattered" && (
          <div style={{ position: "absolute", left: `${scatteredPositions[hoveredStudy].x}%`, top: `${scatteredPositions[hoveredStudy].y - 14}%`, transform: "translateX(-50%)", background: DARK, color: "#FFF", borderRadius: 10, padding: "8px 14px", fontSize: 12, whiteSpace: "nowrap", fontFamily: "'Outfit', 'Noto Sans TC', sans-serif", zIndex: 20, boxShadow: "0 4px 16px rgba(0,0,0,0.15)", pointerEvents: "none" }}>
      ...
      {phase === "scattered" && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 12, justifyContent: "center", marginBottom: 16, fontSize: 12, color: MUTED, fontFamily: "'Outfit', 'Noto Sans TC', sans-serif" }}>
          {studies.map((st) => (
  ...
  return (
    <div style={{ background: CARD_BG, borderRadius: 20, border: `1px solid ${LIGHT_BORDER}`, padding: "32px 24px", boxShadow: "0 2px 20px rgba(0,0,0,0.04)" }}>
      <div style={{ textAlign: "center", marginBottom: 8 }}>
        <h3 style={{ fontFamily: "'Source Serif 4', 'Noto Sans TC', serif", fontSize: 20, fontWeight: 600, color: DARK, marginBottom: 6 }}>{t("forestTitle")}</h3>
        <p style={{ fontSize: 14, color: MUTED, fontFamily: "'Outfit', 'Noto Sans TC', sans-serif" }}>{t("forestClickHint").replace("👆 ", "")}</p>
      </div>
      <div style={{ minHeight: 72, padding: "12px 16px", marginBottom: 16, background: activeTooltip ? `${TEAL}08` : "transparent", borderRadius: 12, transition: "all 0.3s", border: activeTooltip ? `1px solid ${TEAL}22` : "1px solid transparent" }}>
        {activeTooltip ? (
          <>
            <div style={{ fontSize: 14, fontWeight: 600, color: TEAL, fontFamily: "'Outfit', 'Noto Sans TC', sans-serif", marginBottom: 4 }}>{t(explanations[activeTooltip].titleKey)}</div>
            <div style={{ fontSize: 13.5, lineHeight: 1.6, color: MUTED, fontFamily: "'Outfit', 'Noto Sans TC', sans-serif" }}>{t(explanations[activeTooltip].textKey)}</div>
          </>
        ) : (
          <div style={{ fontSize: 13, color: "#B0AFAA", fontFamily: "'Outfit', 'Noto Sans TC', sans-serif", textAlign: "center", paddingTop: 8 }}>
            {t("forestClickHint")}
          </div>
        )}
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", marginLeft: 120, marginRight: 8, marginBottom: 6, fontSize: 10, color: "#B0AFAA", fontFamily: "'Outfit', 'Noto Sans TC', sans-serif" }}>
      ...
      <div style={{ position: "relative" }}>
        {studies.map((s, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", height: 40, padding: "0 4px" }}>
            <div style={{ width: 116, fontSize: 13, color: MUTED, fontFamily: "'Outfit', 'Noto Sans TC', sans-serif" }}>{s.name}</div>
        ...
        <div style={{ display: "flex", alignItems: "center", height: 44, marginTop: 4, borderTop: `1px solid ${LIGHT_BORDER}`, paddingTop: 8 }}>
          <div style={{ width: 116, fontSize: 13, fontWeight: 700, color: TEAL, fontFamily: "'Outfit', 'Noto Sans TC', sans-serif" }}>{t("forestOverall")}</div>
          <div style={{ flex: 1, position: "relative", height: "100%" }}>
      ...
      <div style={{ display: "flex", marginLeft: 120, justifyContent: "space-between", marginTop: 10, fontSize: 10, color: "#C0BFB9", fontFamily: "'Outfit', 'Noto Sans TC', sans-serif" }}>
  ...
  if (finished) {
    ...
    return (
      <div style={{ background: CARD_BG, borderRadius: 20, border: `1px solid ${LIGHT_BORDER}`, padding: "48px 32px", textAlign: "center", boxShadow: "0 2px 20px rgba(0,0,0,0.04)" }}>
        ...
        <h3 style={{ fontFamily: "'Source Serif 4', 'Noto Sans TC', serif", fontSize: 28, fontWeight: 700, color: DARK, marginBottom: 8 }}>{pct === 100 ? t("quizPerfect") : pct >= 60 ? t("quizWellDone") : t("quizKeepLearning")}</h3>
        <p style={{ fontSize: 18, color: TEAL, fontWeight: 600, fontFamily: "'Outfit', 'Noto Sans TC', sans-serif", marginBottom: 8 }}>{t("quizCorrectCount", score, questions.length, pct)}</p>
        <p style={{ fontSize: 14, color: MUTED, marginBottom: 28, fontFamily: "'Outfit', 'Noto Sans TC', sans-serif" }}>{pct === 100 ? t("quizPerfectMsg") : t("quizRetryMsg")}</p>
  ...
  return (
    <div style={{ background: CARD_BG, borderRadius: 20, border: `1px solid ${LIGHT_BORDER}`, padding: "32px 28px", boxShadow: "0 2px 20px rgba(0,0,0,0.04)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <span style={{ fontSize: 12, color: MUTED, fontFamily: "'Outfit', 'Noto Sans TC', sans-serif" }}>{t("quizQuestion")} {current + 1} {t("quizOf")} {questions.length}</span>
      ...
      <h3 style={{ fontFamily: "'Source Serif 4', 'Noto Sans TC', serif", fontSize: 20, fontWeight: 600, color: DARK, marginBottom: 20, lineHeight: 1.4 }}>{q.q}</h3>
      <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 20 }}>
        {q.opts.map((opt, idx) => {
          ...
          return (
            <button key={idx} onClick={() => handleSelect(idx)} style={{ background: bg, border: `1.5px solid ${border}`, borderRadius: 12, padding: "14px 18px", textAlign: "left", fontSize: 15, color, fontFamily: "'Outfit', 'Noto Sans TC', sans-serif", cursor: answered ? "default" : "pointer", transition: "all 0.2s", fontWeight: answered && idx === q.correct ? 600 : 400 }}>
      ...
      {answered && (
        <div style={{ background: selected === q.correct ? "#E6F5F0" : "#FDEEEB", borderRadius: 12, padding: "14px 18px", marginBottom: 16, fontSize: 14, lineHeight: 1.65, color: MUTED, fontFamily: "'Outfit', 'Noto Sans TC', sans-serif", border: `1px solid ${selected === q.correct ? "#3DA87A33" : "#D94B2E33"}` }}>
  ...
  return (
    <div onClick={onClick} style={{ background: CARD_BG, border: `1px solid ${isOpen ? `${TEAL}33` : LIGHT_BORDER}`, borderRadius: 16, padding: "22px 24px", cursor: "pointer", transition: "all 0.35s", boxShadow: isOpen ? `0 4px 24px ${TEAL}0D` : "none" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
        <div style={{ minWidth: 44, height: 44, borderRadius: 12, background: isOpen ? TEAL : "#F1F0EC", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Source Serif 4', 'Noto Sans TC', serif", fontSize: 18, fontWeight: 700, color: isOpen ? "#FFF" : MUTED, transition: "all 0.35s" }}>{number}</div>
        <div style={{ flex: 1 }}>
          <h3 style={{ fontSize: 17, fontWeight: 600, color: DARK, fontFamily: "'Outfit', 'Noto Sans TC', sans-serif", margin: 0 }}>{title}</h3>
          {!isOpen && <p style={{ margin: "4px 0 0", fontSize: 13.5, color: "#B0AFAA", fontFamily: "'Outfit', 'Noto Sans TC', sans-serif" }}>{analogy}</p>}
        </div>
      ...
      {isOpen && (
        <div style={{ marginTop: 16, paddingTop: 16, borderTop: `1px solid ${LIGHT_BORDER}` }}>
          <div style={{ background: `${TEAL}08`, borderRadius: 10, padding: "10px 14px", marginBottom: 14, fontSize: 13.5, color: TEAL, lineHeight: 1.6, fontFamily: "'Outfit', 'Noto Sans TC', sans-serif", fontStyle: "italic" }}>{thinkOfIt}{analogy}</div>
          {details.map((d, i) => (
            <div key={i} style={{ display: "flex", gap: 10, marginBottom: 8, alignItems: "flex-start" }}>
              <span style={{ color: TEAL, fontSize: 7, marginTop: 7 }}>●</span>
              <span style={{ fontSize: 14, lineHeight: 1.65, color: MUTED, fontFamily: "'Outfit', 'Noto Sans TC', sans-serif" }}>{d}</span>
            </div>
          ))}
        </div>
      )}
...
export default function MetaAnalysisGuide() {
  ...
  return (
    <div style={{ background: LIGHT_BG, color: DARK, fontFamily: "'Outfit', 'Noto Sans TC', sans-serif", minHeight: "100vh" }}>
      <style>{`
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, height: 60, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 32px", background: scrollY > 60 ? "rgba(248,247,244,0.92)" : "transparent", backdropFilter: scrollY > 60 ? "blur(16px)" : "none", borderBottom: scrollY > 60 ? `1px solid ${LIGHT_BORDER}` : "none", transition: "all 0.35s" }}>
        <div onClick={() => scrollTo("hero")} style={{ fontFamily: "'Source Serif 4', 'Noto Sans TC', serif", fontSize: 17, fontWeight: 700, color: TEAL, cursor: "pointer", letterSpacing: -0.3 }}>
          {t("navTitle")} <span style={{ fontWeight: 400, color: MUTED }}>{t("navTitleSuffix")}</span>
        </div>
        <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
          {navItems.map((n) => (
            <button key={n.id} onClick={() => scrollTo(n.id)} style={{ background: "none", border: "none", color: MUTED, padding: "8px 14px", borderRadius: 8, fontSize: 13, cursor: "pointer", fontFamily: "'Outfit', 'Noto Sans TC', sans-serif", fontWeight: 500, transition: "color 0.2s" }}
              onMouseEnter={(e) => (e.target.style.color = TEAL)} onMouseLeave={(e) => (e.target.style.color = MUTED)}>
              {n.label}
            </button>
          ))}
          <button onClick={toggleLang} style={{ background: `${TEAL}0D`, border: `1px solid ${TEAL}22`, color: TEAL, padding: "6px 14px", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "'Outfit', 'Noto Sans TC', sans-serif", marginLeft: 8, transition: "all 0.2s", letterSpacing: 0.5 }}
      ...
      <section id="hero" style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ textAlign: "center", padding: "0 24px", maxWidth: 720, zIndex: 1 }}>
          ...
          <FadeIn delay={0.1}>
            <h1 style={{ fontFamily: "'Source Serif 4', 'Noto Sans TC', serif", fontSize: "clamp(36px, 7vw, 64px)", fontWeight: 900, lineHeight: 1.08, marginBottom: 20, color: DARK }}>
              {t("heroTitle1")}
      ...
      <Section id="how" accent>
        <FadeIn><SectionLabel text={t("howLabel")} /><SectionTitle>{t("howTitle")}</SectionTitle>
          ...
          <p style={{ fontSize: 12, color: `${TEAL}88`, marginBottom: 32, fontStyle: "italic", fontFamily: "'Outfit', 'Noto Sans TC', sans-serif" }}>{t("howNote")}</p>
        </FadeIn>
      ...
      <Section id="tools">
        <FadeIn><SectionLabel text={t("forestLabel")} /><SectionTitle>{t("forestTitle")}</SectionTitle>
          ...
          <p style={{ fontSize: 13, color: MUTED, marginBottom: 32, fontFamily: "'Outfit', 'Noto Sans TC', sans-serif" }}>{t("forestNote")}</p>
        </FadeIn>
          ...
          <div style={{ marginTop: 40, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 14 }}>
            {glossary.map((item, i) => (
              <div key={i} style={{ background: CARD_BG, border: `1px solid ${LIGHT_BORDER}`, borderRadius: 14, padding: "18px 20px" }}>
                <h4 style={{ fontSize: 14, fontWeight: 600, color: TEAL, marginBottom: 6, fontFamily: "'Outfit', 'Noto Sans TC', sans-serif" }}>{item.term}</h4>
                <p style={{ fontSize: 13, lineHeight: 1.65, color: MUTED, fontFamily: "'Outfit', 'Noto Sans TC', sans-serif" }}>{item.def}</p>
              </div>
            ))}
          </div>
        </FadeIn>
      ...
      <footer style={{ padding: "48px 24px", textAlign: "center", borderTop: `1px solid ${LIGHT_BORDER}`, background: LIGHT_BG }}>
        <p style={{ fontSize: 13, color: "#B0AFAA", fontFamily: "'Outfit', 'Noto Sans TC', sans-serif", lineHeight: 1.8, maxWidth: 600, margin: "0 auto" }}>
          {t("footerText")}
        </p>
        <div style={{ marginTop: 16, fontFamily: "'Source Serif 4', 'Noto Sans TC', serif", fontSize: 14, color: TEAL, fontWeight: 600 }}>{t("footerBrand")}</div>
      </footer>
    </div>
  );
}
